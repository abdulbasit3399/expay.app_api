        ...
        <section class="section">
          <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
              ...
              <div class="breadcrumb-item active"><a href="{{ route('seller.product-variant',$product->id) }}">{{__('user.Product Variant')}}</a></div>
              <div class="breadcrumb-item">{{__('user.Product Variant Item')}}</div>
            </div>
          </div>
          <div class="section-body">
            <a href="{{ route('seller.product-variant',$product->id) }}" class="btn btn-primary"><i class="fas fa-backward"></i> {{__('user.Go Back')}}</a>

            <a href="javascript:;" data-toggle="modal" data-target="#createVariantItem" class="btn btn-primary"><i class="fas fa-plus"></i> {{__('user.Add New')}}</a>
                      ...
                      <div class="container-fluid">
                        <form action="{{ route('seller.store-product-variant-item') }}" method="POST">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-12">
                                    <label>{{__('user.Price')}} <span>{{__('user.(Set 0 to make it free)')}}</span> <span class="text-danger">*</span></label>
                                ...
                                <form action="{{ route('seller.update-product-variant-item',$variantItem->id) }}" method="POST">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="form-group col-12">
                                            <label>{{__('user.Status')}} <span class="text-danger">*</span></label>
                                            <select name="status" class="form-control">
                                                <option {{ $variantItem->status == 1 ? 'selected' : '' }} value="1">{{__('user.Active')}}</option>
                                                <option {{ $variantItem->status == 0 ? 'selected' : '' }} value="0">{{__('user.Inactive')}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="row">
