...
@section('admin-content')
      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1>{{__('Shipping Rule')}}</h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="{{ route('admin.dashboard') }}">{{__('admin.Dashboard')}}</a></div>
              <div class="breadcrumb-item">{{__('Shipping Rule')}}</div>
            </div>
          </div>
                                        ...
                                        <td>
                                            @if ($shipping->type == 'base_on_price')
                                                @if ($shipping->condition_to == -1)
                                                    {{ $setting->currency_icon. $shipping->condition_from }} - {{__('Unlimited')}}
                                                @else
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                    <div class="modal-header">
                            <h5 class="modal-title">{{__('Create Shipping Rule')}}</h5>
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-12">
                                    ...
                                    <select name="type" id="type_id" class="form-control">
                                        <option value="base_on_price">{{__('Based on product price')}}</option>
                                        <option value="base_on_weight">{{__('Based on product weight (g)')}}</option>
                                        <option value="base_on_qty">{{__('Based on product quantity')}}</option>
      ...
      <div class="modal fade" id="editModelId" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                    <div class="modal-header">
                            <h5 class="modal-title">{{__('Edit Shipping Rule')}}</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                        </div>
