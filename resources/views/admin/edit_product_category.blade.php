...
<div class="main-content">
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-body">
              <h4>Attach Brands</h4>
              <br/>
              <h6>Currenct Serial</h6>
              <p>{{$name}}</p>
              <form method="POST" action="{{route('admin.product-category-brands')}}">
                <input type="hidden" name="category_id" value="{{$category->id}}">
                @csrf
                <div class="form-group">
                  <label>Select {{__('admin.Brands')}} <span class="text-danger">*</span></label>
                  <select class="form-control select2 mult_sort" name="brands[]" multiple="multiple">
                    @foreach($brands as $brand)
                    <option value="{{$brand->id}}" {{in_array($brand->id,$category_brands) ? 'selected' : ''}}>{{$brand->name}}</option>
                    @endforeach
                  </select>
                  <small>Select in serial order (Previous brands will remove with these)</small>
                </div>
                <div class="form-group">
  ...
  <script>
    ...
    $(".mult_sort").on("select2:select", function (evt) {
