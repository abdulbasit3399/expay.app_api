...
class HomeController extends Controller
{
  public function websiteSetup(){
    ...
    $productCategories = Category::with('activeSubCategories.activeChildCategories')->where(['status' => 1])->select('id','name','name_ar','slug','icon')->get();
  ...
  public function sellerDetail(Request $request, $shop_name){
    ...
    $searchCategoryArr = [];
    $searchBrandArr = [];
    $categories = Category::with('activeSubCategories.activeChildCategories')->where(['status' => 1])->select('id','name','name_ar','slug','icon')->get();
  ...
  public function product_filter(Request $request)
  {
    ...
    elseif($request->is_top && $request->is_top != null)
      $products->orderBy('is_top',$request->is_top);
    elseif($request->new_product && $request->new_product != null)
      $products->orderBy('new_product',$request->new_product);
    elseif($request->is_best && $request->is_best != null)
      $products->orderBy('is_best',$request->is_best);

    $get_products = $products->get();


    return response()->json(['products' => $get_products]);
